...
namespace ToolBox
{
    ...
    {
        private static readonly Lazy<MessageSystem> _messageSystem = new Lazy<MessageSystem>();
        public static void AddListener<T>(Action<T> callback) where T : Message => _messageSystem.Value.AddListener(callback);
        public static void RemoveListener<T>(Action<T> callback) where T : Message => _messageSystem.Value.RemoveListener(callback);
        public static void SendMessage<T>(T message) where T : Message => _messageSystem.Value.SendMessage(message);
    }
    ...
    public class MessageSystem
    {
        ...
        public void SendMessage<T>(T message) where T : Message
        {
            if (!_Listeners.TryGetValue(typeof(T), out List<Delegate> methods))
            {
                ...
                {
                    methods[i].Method.Invoke(methods[i].Target, param);
                }
            }
        }
    }
